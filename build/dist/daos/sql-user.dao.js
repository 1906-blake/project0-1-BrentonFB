"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var connection_util_1 = require("../util/connection.util");
var user_converter_1 = require("../util/user.converter");
// export async function findAll() {
//     console.log('finding all users');
//     let client: PoolClient;
//     try {
//         client = await connectionPool.connect(); // basically .then is everything after this
//         const result = await client.query('SELECT * FROM app_user');
//         // convert result from sql object to js object
//         return result.rows.map(convertSqlUser);
//     } catch (err) {
//         console.log(err);
//     } finally {
//         client && client.release();
//     }
//     console.log('found all');
//     return undefined;
// }
function findAll() {
    return __awaiter(this, void 0, void 0, function () {
        var client, queryString, result, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('finding users');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, 5, 6]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 2:
                    client = _a.sent();
                    queryString = "\n        SELECT * FROM users\n        INNER JOIN position USING (positionid)";
                    return [4 /*yield*/, client.query(queryString)];
                case 3:
                    result = _a.sent();
                    return [2 /*return*/, result.rows.map(user_converter_1.convertSqlUser)];
                case 4:
                    err_1 = _a.sent();
                    console.log(err_1);
                    return [3 /*break*/, 6];
                case 5:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 6:
                    console.log('found the users');
                    return [2 /*return*/, undefined];
            }
        });
    });
}
exports.findAll = findAll;
function findById(id) {
    return __awaiter(this, void 0, void 0, function () {
        var client, result, sqlUser, err_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('finding user by id: ' + id);
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, 5, 6]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 2:
                    client = _a.sent(); // basically .then is everything after this
                    return [4 /*yield*/, client.query("\n        SELECT * FROM users\n        INNER JOIN position USING (positionid)\n        WHERE userid = $1", [id])];
                case 3:
                    result = _a.sent();
                    sqlUser = result.rows[0];
                    return [2 /*return*/, sqlUser && user_converter_1.convertSqlUser(sqlUser)];
                case 4:
                    err_2 = _a.sent();
                    console.log(err_2);
                    return [3 /*break*/, 6];
                case 5:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 6: return [2 /*return*/, undefined];
            }
        });
    });
}
exports.findById = findById;
function findByFirstName(firstName) {
    return __awaiter(this, void 0, void 0, function () {
        var client, result, err_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('finding users by first name');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, 5, 6]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 2:
                    client = _a.sent(); // basically .then is everything after this
                    return [4 /*yield*/, client.query('SELECT * FROM app_user WHERE first_name = $1', [firstName])];
                case 3:
                    result = _a.sent();
                    return [2 /*return*/, result.rows.map(user_converter_1.convertSqlUser)];
                case 4:
                    err_3 = _a.sent();
                    console.log(err_3);
                    return [3 /*break*/, 6];
                case 5:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 6: return [2 /*return*/, undefined];
            }
        });
    });
}
exports.findByFirstName = findByFirstName;
function findByUsernameAndPassword(username, password) {
    return __awaiter(this, void 0, void 0, function () {
        var client, queryString, result, sqlUser, err_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, 4, 5]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 1:
                    client = _a.sent();
                    queryString = "\n            SELECT * FROM app_user\n                WHERE username = $1 AND pass = $2\n        ";
                    return [4 /*yield*/, client.query(queryString, [username, password])];
                case 2:
                    result = _a.sent();
                    sqlUser = result.rows[0];
                    return [2 /*return*/, sqlUser && user_converter_1.convertSqlUser(sqlUser)];
                case 3:
                    err_4 = _a.sent();
                    console.log(err_4);
                    return [3 /*break*/, 5];
                case 4:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 5: return [2 /*return*/, undefined];
            }
        });
    });
}
exports.findByUsernameAndPassword = findByUsernameAndPassword;
function save(user) {
    return __awaiter(this, void 0, void 0, function () {
        var client, queryString, params, result, err_5;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, 4, 5]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 1:
                    client = _a.sent(); // basically .then is everything after this
                    queryString = "\n            INSERT INTO app_user (username, pass, first_name, last_name, phone, email, role)\n            VALUES \t($1, $2, $3, $4, $5, $6, $7)\n            RETURNING user_id\n        ";
                    params = [user.username, user.password, user.firstName, user.lastName, user.phone, user.email, user.role];
                    return [4 /*yield*/, client.query(queryString, params)];
                case 2:
                    result = _a.sent();
                    return [2 /*return*/, result.rows[0].user_id];
                case 3:
                    err_5 = _a.sent();
                    console.log(err_5);
                    return [3 /*break*/, 5];
                case 4:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 5:
                    console.log('found all');
                    return [2 /*return*/, undefined];
            }
        });
    });
}
exports.save = save;
function update(user) {
    return __awaiter(this, void 0, void 0, function () {
        var oldUser, client, queryString, params, result, sqlUser, err_6;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, findById(user.id)];
                case 1:
                    oldUser = _a.sent();
                    if (!oldUser) {
                        return [2 /*return*/, undefined];
                    }
                    user = __assign({}, oldUser, user);
                    console.log(user);
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 5, 6, 7]);
                    return [4 /*yield*/, connection_util_1.connectionPool.connect()];
                case 3:
                    client = _a.sent(); // basically .then is everything after this
                    queryString = "\n            UPDATE app_user SET username = $1, pass = $2, first_name = $3, last_name = $4, phone = $5, email = $6, role = $7\n            WHERE user_id = $8\n            RETURNING *\n        ";
                    params = [user.username, user.password, user.firstName, user.lastName, user.phone, user.email, user.role, user.id];
                    return [4 /*yield*/, client.query(queryString, params)];
                case 4:
                    result = _a.sent();
                    sqlUser = result.rows[0];
                    return [2 /*return*/, user_converter_1.convertSqlUser(sqlUser)];
                case 5:
                    err_6 = _a.sent();
                    console.log(err_6);
                    return [3 /*break*/, 7];
                case 6:
                    client && client.release();
                    return [7 /*endfinally*/];
                case 7:
                    console.log('found all');
                    return [2 /*return*/, undefined];
            }
        });
    });
}
exports.update = update;
//# sourceMappingURL=sql-user.dao.js.map